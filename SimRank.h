...
public:
    ...
    ~SimRank()
    {
        ...
        if (frontierPushValueQueue != NULL)
    ...
    void singleSource(int sid)
    {
        ...
        cout << "MaxLevel = " << maxLevToPush << endl;

        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double, std::milli> fp_ms = end - start;
        cout << "cpu detectmax time:" << fp_ms.count() << " ms" << endl;

        auto start1 = std::chrono::high_resolution_clock::now();

        vector<mymapOfVecI> parentsOfNodesInGu(maxLevToPush + 2); // store each level previous node

        int levelOfPush = 0;
        vector<mymapID> hprobsFromS(maxLevToPush + 2); // store each level hitting probs
        ...
        std::chrono::duration<double, std::milli> fp_ms1 = end1 - start1;
        cout << "cpu frontier time:" << fp_ms1.count() << " ms" << endl;
        ...
        kernel_wrapper(sid, epsilon_h, g, sqrtC, sidIndegRecip, hprobsFromS, parentsOfNodesInGu);
        ...
        for (int childLevelInGu = lastLevWithAttNode; childLevelInGu >= 2; --childLevelInGu)
        {
            ...
            for (auto probTree : probTreesOfAllNodes[childLevelInGu])
            {
                ...
                for (auto pid : parentsOfNodesInGu[childLevelInGu][childid])
                {
                    ...
                    {
                        if (childProbTree[stepInChild].size() == 0)
                            continue; // filter attention node(probsFromNode[0][nid] = 1.0)
                        for (auto probEntry : childProbTree[stepInChild])
                        {
                            probTreesOfAllNodes[parentLevInGu][pid][stepInChild + 1][probEntry.first] += probEntry.second * onestepProb;
                        }
                    }
                }
            }
